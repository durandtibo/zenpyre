...
def make_ingestor(data: list, n: int) -> LastNIngestor:
    return LastNIngestor(ingestor=InMemoryIngestor(data=data), n=n)
...
def test_last_n_ingestor_stores_ingestor() -> None:
    ...
    assert LastNIngestor(ingestor=inner, n=2)._ingestor is inner


def test_last_n_ingestor_stores_n() -> None:
    assert LastNIngestor(ingestor=InMemoryIngestor(data=[]), n=3)._n == 3
...
def test_last_n_ingestor_raises_for_non_positive_n(n: int) -> None:
    with pytest.raises(ValueError, match="n must be a positive integer"):
        LastNIngestor(ingestor=InMemoryIngestor(data=[]), n=n)
...
def test_last_n_ingestor_repr_contains_n() -> None:
...
def test_last_n_ingestor_ingest_accepts_tuple_sequence() -> None:
    ingestor = LastNIngestor(ingestor=InMemoryIngestor(data=(1, 2, 3, 4)), n=2)
