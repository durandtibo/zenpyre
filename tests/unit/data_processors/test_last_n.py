...
def test_last_n_processor_n_zero_raises() -> None:
    with pytest.raises(ValueError, match=r"n must be greater than or equal to 1, got 0"):
        LastNProcessor(n=0)


def test_last_n_processor_n_negative_raises() -> None:
    with pytest.raises(ValueError, match=r"n must be greater than or equal to 1, got -1"):
        LastNProcessor(n=-1)
