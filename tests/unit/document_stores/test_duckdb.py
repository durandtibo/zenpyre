...
@duckdb_available
def test_filter_integer_metadata_value(store: DuckDBDocumentStore, docs: list[Document]) -> None:
    store.add_documents(docs)
...
def test_context_manager_multiple_open_close_in_memory() -> None:
    doc_store = DuckDBDocumentStore(":memory:")
    for i in range(3):
...
def test_context_manager_multiple_open_close_persistent(tmp_path: Path) -> None:
    doc_store = DuckDBDocumentStore(tmp_path.joinpath("data.db"))
    for i in range(3):
        with doc_store as store:
            store.add_documents([Document(id=str(i), page_content="hello", metadata={})])
