...
@pytest.fixture
def store() -> Generator[TypedSQLiteRecordStore, None, None]:
    """In-memory store with no schema."""
    with TypedSQLiteRecordStore(":memory:") as store:
        yield store


@pytest.fixture
def typed_store() -> Generator[TypedSQLiteRecordStore, None, None]:
    """In-memory store with a typed schema."""
    with TypedSQLiteRecordStore(
        ":memory:",
...
) -> Generator[TypedSQLiteRecordStore, None, None]:
    path = store_path / "data.sqlite"
    store = TypedSQLiteRecordStore.from_path(path)
    store.add_records(records)
    store._conn.close()
    with TypedSQLiteRecordStore.from_path(path, read_only=True) as store:
        yield store
...
def test_init_defaults_to_in_memory() -> None:
    with TypedSQLiteRecordStore() as store:
        assert store.count() == 0


def test_init_accepts_sqlite_connect_kwargs() -> None:
    with TypedSQLiteRecordStore(":memory:", timeout=5.0) as store:
        assert store.count() == 0


def test_init_no_schema_stores_everything_in_extra(store: TypedSQLiteRecordStore) -> None:
    store.add_records([Record(id="1", metadata={"author": "Alice"})])
    assert set(store.get_columns_info().keys()) == {"id", "extra"}


def test_init_with_schema_creates_typed_columns(typed_store: TypedSQLiteRecordStore) -> None:
    columns = typed_store.get_columns_info()
    assert set(columns.keys()) == {"id", "author", "year", "category", "extra"}
...
def test_from_path_creates_file_backed_store(store_path: Path) -> None:
    ...
    with TypedSQLiteRecordStore.from_path(path) as store:
        store.add_records([Record(id="1", metadata={"a": 1})])
        assert store.count() == 1
        assert path.exists()


def test_from_path_forwards_metadata_schema(store_path: Path) -> None:
    path = store_path / "from_path_schema.sqlite"
    schema = {"author": "TEXT"}
    with TypedSQLiteRecordStore.from_path(path, metadata_schema=schema) as store:
        assert "author" in store.get_columns_info()


def test_from_path_memory_uses_shared_cache_uri() -> None:
    with TypedSQLiteRecordStore.from_path(":memory:") as store:
        assert store.count() == 0


def test_from_path_read_only_can_read_existing_data(
    store_read_only: TypedSQLiteRecordStore,
) -> None:
    assert store_read_only.count() == 4


def test_from_path_read_only_rejects_writes(store_read_only: TypedSQLiteRecordStore) -> None:
...
def test_init_read_only_connection_with_existing_table_does_not_raise(
    store_read_only: TypedSQLiteRecordStore, records: list[Record]
) -> None:
...
def test_init_read_only_connection_without_existing_table_swallows_operational_error(
    ...
    # table, bypassing TypedSQLiteRecordStore entirely.
    ...
    with (
        TypedSQLiteRecordStore.from_path(path, read_only=True) as store,
...
def test_repr(store: TypedSQLiteRecordStore) -> None:
    assert repr(store).startswith("TypedSQLiteRecordStore(")


def test_str(store: TypedSQLiteRecordStore) -> None:
    assert repr(store).startswith("TypedSQLiteRecordStore(")
...
def test_add_records_increases_count(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    assert store.count() == len(records)


def test_add_records_upsert_replaces_existing(store: TypedSQLiteRecordStore) -> None:
...
def test_add_records_empty(store: TypedSQLiteRecordStore) -> None:
    store.add_records([])
...
def test_count_empty_store(store: TypedSQLiteRecordStore) -> None:
    assert store.count() == 0


def test_count_after_adding(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    assert store.count() == len(records)
...
def test_get_existing_record(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    result = store.get("1")
    assert result == records[0]


def test_get_missing_record_returns_none(store: TypedSQLiteRecordStore) -> None:
...
def test_get_round_trips_extra_metadata(typed_store: TypedSQLiteRecordStore) -> None:
...
def test_get_many_preserves_order(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_filter_extra_field(typed_store: TypedSQLiteRecordStore) -> None:
    records = [
...
def test_filter_mixed_schema_and_extra_fields(typed_store: TypedSQLiteRecordStore) -> None:
    records = [
        Record(id="1", metadata={"author": "Alice", "publisher": "O'Reilly"}),
...
def test_delete_removes_record(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    store.delete("1")
...
def test_delete_nonexistent_is_silent(store: TypedSQLiteRecordStore) -> None:
...
def test_delete_many_removes_records(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_delete_many_nonexistent_ids_are_silent(store: TypedSQLiteRecordStore) -> None:
    store.delete_many(["99", "100"])


def test_delete_many_single_id(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    store.delete_many(["2"])
    assert store.count() == len(records) - 1
...
def test_check_ids_all_found(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_check_ids_all_missing(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_check_ids_mixed(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_check_ids_preserves_order(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_check_ids_empty_input_returns_empty_lists(store: TypedSQLiteRecordStore) -> None:
    found, missing = store.check_ids([])
    assert found == []
    assert missing == []


def test_check_ids_empty_store_returns_all_missing(store: TypedSQLiteRecordStore) -> None:
...
def test_all_empty_store(store: TypedSQLiteRecordStore) -> None:
    assert store.all() == []


def test_all_returns_all_records(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
    result = store.all()
...
def test_lazy_all_empty_store_yields_nothing(store: TypedSQLiteRecordStore) -> None:
    assert list(store.lazy_all()) == []


def test_lazy_all_returns_generator(store: TypedSQLiteRecordStore) -> None:
...
def test_lazy_all_returns_all_records(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_lazy_all_matches_all(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_lazy_all_preserves_metadata(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_lazy_all_single_record(store: TypedSQLiteRecordStore) -> None:
...
def test_iter_batches_empty_store_yields_nothing(store: TypedSQLiteRecordStore) -> None:
    assert list(store.iter_batches()) == []


def test_iter_batches_returns_generator(store: TypedSQLiteRecordStore) -> None:
...
def test_iter_batches_batch_size_one(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_iter_batches_matches_all(store: TypedSQLiteRecordStore, records: list[Record]) -> None:
    store.add_records(records)
...
def test_iter_batches_zero_batch_size_raises(store: TypedSQLiteRecordStore) -> None:
    with pytest.raises(ValueError, match="batch_size must be a positive integer"):
        list(store.iter_batches(batch_size=0))


def test_iter_batches_negative_batch_size_raises(store: TypedSQLiteRecordStore) -> None:
    with pytest.raises(ValueError, match="batch_size must be a positive integer"):
        list(store.iter_batches(batch_size=-1))


def test_iter_batches_error_raised_before_any_query(store: TypedSQLiteRecordStore) -> None:
...
def test_get_columns_info_returns_dict(store: TypedSQLiteRecordStore) -> None:
    result = store.get_columns_info()
    assert isinstance(result, dict)


def test_get_columns_info_values_are_strings(store: TypedSQLiteRecordStore) -> None:
    result = store.get_columns_info()
    assert all(isinstance(v, str) for v in result.values())


def test_get_columns_info_matches_pragma_output(store: TypedSQLiteRecordStore) -> None:
...
def test_get_columns_info_non_empty_for_created_table(store: TypedSQLiteRecordStore) -> None:
...
def test_get_columns_info_does_not_mutate_between_calls(store: TypedSQLiteRecordStore) -> None:
...
def test_show_columns_info_does_not_raise(
    store: TypedSQLiteRecordStore, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_show_columns_info_returns_none(store: TypedSQLiteRecordStore) -> None:
...
def test_close_closes_underlying_connection(store: TypedSQLiteRecordStore) -> None:
    store.close()
    with pytest.raises(sqlite3.ProgrammingError, match=r"closed database"):
        store.count()


def test_close_is_idempotent(store: TypedSQLiteRecordStore) -> None:
    store.close()
    store.close()  # should not raise


def test_close_returns_none(store: TypedSQLiteRecordStore) -> None:
...
def test_context_manager_closes_on_exception() -> None:
    msg = "boom"
    with pytest.raises(ValueError, match="boom"), TypedSQLiteRecordStore(":memory:") as store:
...
def test_context_manager_usable_for_reads_and_writes() -> None:
    with TypedSQLiteRecordStore(":memory:", metadata_schema={"author": "TEXT"}) as store:
        store.add_records(
            [
